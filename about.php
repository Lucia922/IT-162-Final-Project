<?php 
include 'includes/header.php';
?>

    <section class="pic"> 
        <h2>РЧКЧ СВЕЛО</h2>
    </section>

    <div class="manifesto"> 
        <h3 > The Manifesto </h3>
        <p>
            Ruku Svelo,roughly translated from Russian,means "Blossoming hand."
            My two daughters are the inspiration behind my designs.
            I want my daughters to lead beautiful lives,
            to enjoy sophisticated aesthetics,
            and to be able to navigate emotional complexity.
        </p>  
        <p>                                     
            I have been dreaming of children's clothes that are
            classic, sturdy, comfortable, simple, wholesome, and honest.
            Ruku Svelo celebrates traditions, the passage of time,
            and the far-away future.
        </p>
    </div>

    <div class="ella">
       <img src="images/ella.jpg"  style="width:100%;" alt=""/>
    </div>

    <div class="about">   
        <h3> About </h3>
        <p>
            The birth of Ruku Svelo began with my mother-in-law Ella's wardrobe,
            which is full of linen dresses, wool stockings, hand-printed cottons,
            hand-knitted sweaters, and the old, soft, worn textures of Soviet Russia.
        </p>   
        <p>                                                         
            As a designer, I worked for ten years in women's apparel.
            But I quit fast fashion and throw-away culture.
            Now most of my designs are one-offs, made from rare fabrics,
            hand-selected for quality, designed to be passed down through several generations.
        </p>  
        <p> 
            I live with my husband and two daughters in Seattle.
            We are surrounded by water, mountains,
            and evergreen forests.                                                        
        </p> 
    </div> 
    

    
<?php
include 'includes/footer.php';
?> 

         